...
pub fn prelude() -> IndexMap<Ident, Global> {
    ...
    prelude.insert(Ident::tid("Bool"), Global::Lit(Lit::BoolT));

    // Prelude values
    prelude.insert(Ident::vid("true"), Global::Lit(Lit::True));
    prelude.insert(Ident::vid("false"), Global::Lit(Lit::False));
